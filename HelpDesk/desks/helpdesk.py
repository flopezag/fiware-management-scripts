...
class HelpDesk:
    ...
    def _change_channel(self):
        ...
        for issue in filter(condition, requests):
            ...
            info('update issue= {} - change to LAB channel')
        ...
        for issue in filter(condition, requests):
            ...
            info('update issue= {} - change to TECH channel'.format(issue))
    ...
    def _assign_tech_channel(self):
        ...
        for issue in filter(condition, requests):
            ...
            info('update issue: issue= {} node={}, chapter={}, enabler={}'.format(issue,
                                                                                          node['value'],
                                                                                          chapter['value'],
                                                                                          enabler['value']))
        ...
        for issue in filter(condition, requests):
            ...
            if enabler in self.enablersBook:
                ...
                info('assign issue= {} enabler={} chapter={} assignee={}'
        ...
        for issue in filter(condition, requests):
            ...
            if chapter in self.chaptersBook:
                ...
                assignee = component.assignee.name
                self.jira.assign_issue(issue, assignee=assignee)
                info('assign issue= {} chapter={} assignee={}'.format(issue, chapter, assignee))
            else:
                message = 'Dear Help Desk Caretaker Admin,' +\
    ...
    def _assign_lab_channel(self):
        ...
        for issue in filter(condition, requests):
            ...
            info('update issue= {} node={} enabler={}'.format(issue, node['value'], enabler['value']))
        ...
        for issue in filter(condition, requests):
            ...
            if node in self.nodesBook:
                ...
                self.jira.assign_issue(issue, assignee=assignee)
                info('assign issue= {} node={} assignee={}'.format(issue, node, assignee))
            else:
                message = 'Dear Help Desk Caretaker Admin,' +\
    ...
    def channel_requests(self):
        ...
        for request in requests:
            ...
            if match:
                ...
                info('updated request {}, channel= {}'.format(request, channel))
    ...
    def naming(self):
        ...
        for issue in filter(condition, issues):
            if not match(r'SPAM\s=>', issue.fields.summary):
                summary = 'SPAM => ' + sub(r'\[[^\]]+?\]', '', issue.fields.summary.strip())
        ...
        for issue in filter(condition, issues):
            ...
            # formato correcto
            if match(pattern, issue.fields.summary):
                continue

            # en canal equivocado
            gpattern = r'FIWARE\.{}\.({})\.'.format(issuetype, _channels)

            if match(gpattern, issue.fields.summary.strip()):
                summary = sub(r'\.({})\.'.format(_channels), chkeyword, issue.fields.summary).strip()
            else:
                summary = sub(r'\[[^\]]+?\]', '', issue.fields.summary.strip())
                summary = sub(r'\.FIWARE\.(Request|Question)\.\w+\.', '', summary)
        ...
        for issue in filter(condition, issues):
            ...
            if enabler_value != 'Unknown':
                try:
                    ...
                    exception('Unknown enabler {}'.format(enabler_value))
                    continue
    ...
    def remove_spam(self):
        ...
        for request in requests:
            request.delete()
            info('DELETED SPAM {}'.format(request))
            self.n_removed += 1
